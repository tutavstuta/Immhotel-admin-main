<template>
  <div class="card">
    <DataTable :value="roomOverviewData" tableStyle="min-width: 50rem" :rowHover="true">
      <template #header>
        <div class="flex flex-wrap align-items-center justify-content-between gap-2">
          <span class="text-xl text-900 font-bold">รายการบริการ</span>
      ...
      <template #footer>
        รวมทั้งหมด {{ roomOverviewData ? roomOverviewData.length : 0 }} รายการ.
      </template>
    </DataTable>
    <!-- Dialog create -->
...
export default {
  ...
  methods: {
    ...
    async getById(id) {
      await this.roomOverviewService.getById(id).then((result) => {
        if (result && result.message === "get room overview successfully") {
    ...
    async update() {
      ...
      await this.roomOverviewService
    ...
    async delete(id) {
      await this.roomOverviewService.delete(id).then(async (result) => {
        if (result) {
          this.$toast.add({
            severity: "success",
